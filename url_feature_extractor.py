...
"""
Railway-optimized URL feature extractor (fast, non-blocking).
Sets:
 - TRAIN_MODE: provide full features (slower; for offline batch)
 - FAST_MODE: skip slow external checks (for production web API)
Usage:
    from url_feature_extractor import extract_all_features
    features = extract_all_features("https://example.com")
"""
import os
import re
import time
...
# Small in-memory cache for repeated domain lookups
_CACHE = {}
...
# ---- NETWORK FUNCTIONS (fast timeouts, safe) ----
def safe_request(url, timeout=2, verify=False, max_bytes=150000):
    """Lightweight GET with small timeout; returns text or None."""
    if not requests:
        return None
    try:
...
def safe_whois(host, timeout=3):
    """Return domain age in days or None. In FAST_MODE returns a safe default."""
    if FAST_MODE:
        return None  # prefer fallback age in feature builder
    if not pywhois:
        return None
    try:
        # whois library can be slow; wrap in timeout via cache + try/except
        cached = cache_get(f"whois::{host}")
        ...
        if isinstance(cd, str):
            ...
            try:
                ...
                cd = None
        if cd:
            days = max(0, (datetime.utcnow() - cd).days)
            cache_set(f"whois::{host}", days)
            return days
    except Exception:
        return None
    return None

def safe_ssl_valid(host, timeout=1.5):
    """Check SSL cert quickly; return True/False/None."""
    if FAST_MODE:
        return True  # most sites have cert now; assume valid in FAST
    try:
        ctx = ssl.create_default_context()
        # short connect timeout
        with socket.create_connection((host, 443), timeout=timeout) as s:
            with ctx.wrap_socket(s, server_hostname=host) as ss:
                return bool(ss.getpeercert())
    except Exception:
        return None

def safe_quad9_blocked(host, lifetime=1.2):
    """Return 0 if resolved, 1 if blocked/unresolvable, or None."""
    if FAST_MODE:
        return 0
    if not dns:
        return None
    try:
        cached = cache_get(f"quad9::{host}")
        if cached is not None:
            return cached
        r = dns.resolver.Resolver(configure=False)
        r.nameservers = ["9.9.9.9"]
        r.resolve(host, "A", lifetime=lifetime)
        cache_set(f"quad9::{host}", 0)
        return 0
    except Exception:
        cache_set(f"quad9::{host}", 1)
        return 1

def vt_domain_info(url_or_host, timeout=2):
    """Return (total, malicious, ratio) or zeros. FAST_MODE disables remote VT."""
...
def extract_all_features(url: str) -> dict:
    ...
    features["entropy_url"] = float(shannon_entropy(u))
    free_hosts = ["webflow.io","wixsite.com","weebly.com","000webhost","firebaseapp.com","github.io","shopify","wordpress.com","blogspot.com"]
    ...
    # TRAIN_MODE or FAST: skip slow live calls or provide safe defaults
    if TRAIN_MODE:
        # full offline batch - try to fill everything if possible
        domain_age = safe_whois(host) or 365
        features["domain_age_days"] = int(domain_age)
        ssl_ok = safe_ssl_valid(host)
        features["ssl_valid"] = int(ssl_ok) if ssl_ok is not None else 1
        features["quad9_blocked"] = int(safe_quad9_blocked(host) or 0)
        ...
        # FAST/production mode: keep defaults and avoid blocking
        domain_age = safe_whois(host)
        features["domain_age_days"] = int(domain_age) if domain_age is not None else 365
        ssl_ok = safe_ssl_valid(host)
        features["ssl_valid"] = int(ssl_ok) if ssl_ok is not None else 1
    ...
    # HTML features (best-effort with short timeout)
    html = safe_request(u, verify=False, timeout=2)
    ...
    if soup:
        ...
        try:
            ...
            for f in soup.find_all("form"):
                ...
                if "password" in inputs or "login" in (f.text or "").lower():
                    login = 1
                    break
            features["login_form"] = int(login)
        except Exception:
    ...
    # Final expected ordering (makes later vectorization stable)
    expected = [
