...
Now improved:
- VirusTotal now cached (simple_cache)
- No sleep() blocking
- Safer request streaming (max 200 KB)
- TLS warnings disabled (optional)
- More resilient WHOIS/SSL/DNS fallbacks
"""
...
# --------------------------------------
# Suppress SSL/TLS warnings (optional)
# --------------------------------------
try:
    import urllib3
...
try:
    ...
    # fallback in memory (no cross-run persistence)
    _CACHE = {}
...
def vt_scan_info(url_or_host):
    """
    Returns (total, malicious, ratio)
    Domain → fast primary
    URL → fallback
    Fully cached to avoid rate limits.
    """
    ...
    # --- URL lookup (fallback, no sleep) ---
    try:
        cache_key2 = f"vt_url::{domain}"
        cached2 = cache_get(cache_key2, max_age=3600)
        if cached2:
            return cached2["total"], cached2["malicious"], cached2["ratio"]
        ...
        if r.status_code not in (200, 202):
            cache_set(cache_key2, {"total": 0, "malicious": 0, "ratio": 0})
            return 0, 0, 0.0
        ...
        for _ in range(3):
            ...
            if s.status_code == 200:
                stats = s.json().get("data", {}).get("attributes", {}).get("stats", {})
                if isinstance(stats, dict):
                    total = sum(stats.values())
                    malicious = stats.get("malicious", 0)
                    ratio = malicious / total if total > 0 else 0.0
                    cache_set(cache_key2, {"total": total, "malicious": malicious, "ratio": ratio})
                    return total, malicious, ratio
...
def safe_request(url, timeout=6, verify=False, max_bytes=200000):
    """
    Request HTML safely, streaming with size cap (200 KB).
    """
    if requests is None:
        return None
    try:
...
def extract_all_features(url):
    ...
    # ---------------------------
    # FINAL: ensure all expected keys
    # ---------------------------
    expected = [
