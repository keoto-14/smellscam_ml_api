    ...
    try:
        ...
        if isinstance(cd, str):
            try:
                ...
                # try parsing common formats fallback
                cd = None
        if cd:
...
def safe_ssl(host):
    ...
    try:
        ...
        # assume valid to avoid false negatives if socket failed transiently
        return 1
...
def check_dns_exists(host):
    """Return 1 if DNS A record resolves, 0 otherwise. If dns lib missing, return 1 (assume exists)."""
    if not dns:
        return 1
    try:
...
# ---------------------------------------------------------
# Marketplace Detector (SAFE, string-based)
# ---------------------------------------------------------
def detect_marketplace(host):
    # match common marketplace host substrings
...
# ---------------------------------------------------------
# Seller Detector (SAFE — does not alter ML features)
# - Input: url lowercase string and marketplace_type (int)
# - Output: 0 = Unknown/Not-applicable, 1 = Verified, 2 = Suspicious
# ---------------------------------------------------------
def detect_seller_status(url_l: str, marketplace_type: int) -> int:
    if marketplace_type == 0:
        return 0  # Not a marketplace link — don't surface seller

    # Verified patterns
    if any(tok in url_l for tok in ("official", "flagship", "verified")):
        return 1

    # Suspicious patterns — e.g., 'seller' used in weird contexts without standard shop/item markers
    # (Keep conservative: mark suspicious only if clearly odd)
    if "seller" in url_l and ("id=" not in url_l and "shop" not in url_l and "store" not in url_l):
        return 2

    # Normal seller / unknown (shop/store/item pages)
    if any(tok in url_l for tok in ("shop", "store", "seller", "item", "product")):
        return 0
...
def extract_all_features(url):
    ...
    # brand shared words between host and path
    ...
    if FAST_MODE or TRAIN_MODE:
        f.update({
            ...
            "domain_age_days": 365,
            "quad9_blocked": 0,
            # keep vt fields neutral here; predictor will call VT
            "vt_total_vendors": 0,
        ...
        # vt fields left neutral here; predictor will call VT/GSB
        f["vt_total_vendors"] = 0
        f["vt_malicious_count"] = 0
        f["vt_detection_ratio"] = 0.0

    # ---------------------------------------------------------
    # Marketplace & Seller & Domain Exists (SAFE: does not change ML core)
    # ---------------------------------------------------------
    f["marketplace_type"] = detect_marketplace(host)

    # Seller only applied when marketplace detected (0 otherwise)
    f["seller_status"] = detect_seller_status(url_l, f["marketplace_type"]) if f["marketplace_type"] != 0 else 0

    # DNS existence (very important safety signal)
    f["domain_exists"] = check_dns_exists(host)

    # ---------------------------------------------------------
    # FIXED ORDER — must match features.pkl order used by the model
    # (we put marketplace_type, seller_status, domain_exists last)
    # ---------------------------------------------------------
    expected = [
